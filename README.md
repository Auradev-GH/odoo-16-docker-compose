...
``` bash
curl -s https://raw.githubusercontent.com/Auradev-GH/odoo-16-docker-compose/main/run.sh | sudo bash -s odoo-one 10016 20016
```
and/or run the following to set up another Odoo instance @ `localhost:11016` (default master password: `auradev`):

``` bash
curl -s https://raw.githubusercontent.com/Auradev-GH/odoo-16-docker-compose/main/run.sh | sudo bash -s odoo-two 11016 21016
```
...
* Default database password (**admin_passwd**) is `auradev`, please change it @ [etc/odoo.conf#L60](/etc/odoo.conf#L60)
